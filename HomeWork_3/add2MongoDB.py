...
def _parser_item_superjob(item):
    ...
    vacancy_date['city'] = str(company_location)
...
def add2Mongo(vacancy):

    vacancy_date = []
    vacancy_date.extend(_parser_hh(vacancy))
    vacancy_date.extend(_parser_superjob(vacancy))
    print(_parser_hh(vacancy)['vacancy_name'])

    for item in vacancy_date:
        if bool(db.hh.find(item)):
            db.hh.update_one({'vacancy_link': item['vacancy_link']}, {'$set': item})
        else:
            print('test')
            db.hh.insert_one(item)

vacancy = 'python'
mg = add2Mongo(vacancy)
